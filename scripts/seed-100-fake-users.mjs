...
const firstNames = ['Stan', 'Cris', 'Jacob', 'Daniel', 'Robby', 'Grant', 'Maya', 'Leo', 'Ivy', 'Max', 'Nina', 'Omar', 'Pia', 'Quinn', 'Rhea', 'Sage', 'Theo', 'Uma', 'Vera', 'Wren'];
const lastNames = ['Mercer', 'Soto', 'Jacquet', 'Caridi', 'Frank', 'Cooper', 'Chen', 'Park', 'Brooks', 'Rivera', 'Patel', 'Singh', 'Mori', 'Khan', 'Flores', 'Miller', 'Wong', 'Davis', 'Young', 'Stone'];
const handlePrefixes = ['stan', 'trim', 'rezi', 'kibu', 'capture', 'comet', 'ship', 'forge', 'neon', 'grep', 'stack', 'byte', 'mono', 'flux', 'pixel', 'scope', 'patch', 'script', 'modal', 'radar'];
const handleSuffixes = ['builds', 'ops', 'loop', 'labs', 'works', 'forge', 'pilot', 'flow', 'grid', 'stack', 'shift', 'core', 'mode', 'craft', 'sync', 'mint', 'beam', 'dock', 'spark', 'arena'];
const avatarSeeds = ['stan', 'cris', 'jacob', 'daniel', 'robby', 'grant', 'maya', 'leo', 'ivy', 'max', 'nina', 'omar', 'pia', 'quinn', 'rhea', 'sage', 'theo', 'uma', 'vera', 'wren'];
...
const fakeUsers = Array.from({ length: 100 }, (_, index) => {
  ...
  const prefix = handlePrefixes[index % handlePrefixes.length];
  const suffix = handleSuffixes[(index * 7) % handleSuffixes.length];
  const handle = `${prefix}${suffix}${index + 1}`.toLowerCase();
  const xHandle = index % 9 === 0 ? undefined : `${prefix}${suffix}${(index % 30) + 1}`.toLowerCase();
  const avatar = `${avatarSeeds[index % avatarSeeds.length]}-${index + 1}`;
  const scale = Math.round(2_250_000 - index * 18_500);
  const activityBias = 0.78 + ((index % 9) * 0.035);
  const recentBoost = 0.72 + ((index % 11) * 0.09);
  const providers = index % 4 === 0 ? ['claude'] : index % 5 === 0 ? ['codex'] : ['claude', 'codex'];
...
const snapshots = fakeUsers.map((user, index) => {
  ...
  return {
    ...
    capturedAt: isoAtUtc(index % 5, 9 + (index % 10)),
    submittedAt: isoAtUtc(index % 4, 10 + (index % 8)),
    windowDays: 365,
    cliVersion: '0.1.2',
    profile: {
      provider: 'github',
      providerUserId: `fake-gh-${2000 + index}`,
      handle: `@${user.handle}`,
